...
- `int rfid_create_mask_from_value` - Generates the packet mask based on the package received
- `int rfid_check_command` - Checks if the received command is valid and present on the EPC-GEN2 protocol
- `int rfid_get_ip_id` - Checks the currend address for the IP_Core
...
- `sender_check_usedw` - Access the address that indicates how many packets are in the sender FIFO
...
- `sender_get_command_ints_size` - Check the size of the command and calculates the size of each smaller packet
- `sender_add_mask` - Divides the command into smaller packets if needed and generates a mask based on the current packet data size
...
- `receiver_check_usedw` - Access the address that indicates how many packets are in the receiver FIFO
- `receiver_request_package` - Access BASE_RECEIVER_DATA to read the received packet
...
- `receiver_get_package` - Separates the packet from `receiver_request_package` into data and mask