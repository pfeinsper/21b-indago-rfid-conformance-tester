...
The NIOS II soft processor we use in our project was implemented by our team and is based on RISC architecture. It is responsible for generating all the commands we send to the TAG, as well as interpreting the responses our device receives. More information on the NIOS II can be found [here](firmware.md).
