...
All necessary VHDL hardware description files are located in the project’s <guide>fpga/RTL</guide> folder. The top-level entity of the entire processor that includes all the required configuration generics is the <guide>RTL/rfid.vhd</guide> file.
...
- IP variation file type <guide>VHDL</guide>;
